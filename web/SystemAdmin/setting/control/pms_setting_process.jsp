...
<%
    ...
    String query = "select status from adm_system_parameter where system_code = 'IT' and parameter_code ='1' and hfc_cd='"+hfc_cd+"';";
    ...
    if(dataPara.size()<1){ //No data yet, so we insert new row of data
        ...
        query ="Insert into adm_system_parameter(system_code, parameter_code, parameter_name, status, created_by, created_date, hfc_cd) "
                + "values('IT', '1', 'Implementation_type', '"+type+"', '"+creator+"', now(), '"+hfc_cd+"');";
        ...
        query="Update adm_system_parameter set status='"+type+"' "
                + "where hfc_cd='"+hfc_cd+"' and system_code='IT' and parameter_code='1'";
        success = rmi.setQuerySQL(con.HOST, con.PORT, query);
    }
    ...
    if(success){
        out.print("success");
        //set session
        session.setAttribute("SYSTEM_PARAMETER", type);
    }
    else{
        out.print("fail");
    }
