<%-- 
    Document   : reportBillingSalesReportGetMonthlyItemSalesReport
    Created on : Feb 23, 2018, 10:40:41 AM
    Author     : Shammugam
--%>
...
<%
    ...
    String sql = " SELECT cd.item_cd, cd.item_desc, SUM(cd.quantity) AS SQ, SUM(cd.item_amt) AS SA"
            + " FROM far_customer_dtl cd "
            + " LEFT JOIN far_customer_hdr ch ON (cd.bill_no = ch.bill_no)  "
            + " WHERE MONTH(cd.txn_date) = '" + month + "' AND YEAR(cd.txn_date) = '" + year + "' AND ch.hfc_cd = '" + hfc_cd + "'  "
            + " GROUP BY cd.item_cd  "
            + " ORDER BY SQ DESC ";
...
<div class="center-block"  style="text-align: center;">
    <h2>
        Monthly Sales Report for <%=monthString%> <%=year%>
    </h2>
</div>
<br>
<div class="center-block" style="align-content: center;">
    <table class="table table-filter table-striped table-bordered" width="100%" cellpadding="5" border="1" id="reportBilllingItemSalesReportTable">
        <thead>
            <tr>
                <th width="10%">No.</th>
        ...
        <tbody> 
            <%
                for (int i = 0; i < data.size(); i++) {
                    ...
                    String itemCode = data.get(i).get(0);
                    String itemName = data.get(i).get(1);
...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready(function () {
        ...
        $('#reportBilllingItemSalesReportTable').DataTable({
            pageLength: 10,
            dom: 'Bfrtip',
            buttons: [
                {
                    ...
                    extend: 'print',
                    text: 'Print Monthly Item Sales List',
                    title: $('h1').text(),
                    message: '<br><br>',
                    className: 'btn btn-primary',
                    customize: function (win) {
                        $(win.document.body)
                                ...
                                .prepend(
                                        ...
                                        <img src="<%=dataLogo.get(0).get(0)%>" style="text-align: center; height: 100%; " /></div> <div class="mesej"><br>Monthly Item Sales List for <%=monthString%> <%=year%></div>\n\
                                        <div class="info_kecik">\n\
                                        <dd>Date: <strong><%=newdate%></strong></dd>\n\
                                        <dd>Report No: <strong>BLI-0008</strong></dd>\n\
                                        </div> '
                                        );
